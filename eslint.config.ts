...
const combineConfig = (...configs: Config[][]) => configs.flatMap(part => part)
...
const onlyTypeChecked = (config: Config) => ({
	...config,
	files: fileGlobs.ts,
})
...
const svelte = combineConfig(
	sveltePlugin.configs['flat/recommended'],
	sveltePlugin.configs['flat/prettier'],
	[
		{
			files: fileGlobs.svelte,
			languageOptions: {
				parserOptions: {
					parser: tsParser,
				},
...
const importX: Config[] = [
	// don't forget to install 'eslint-import-resolver-typescript'
			...
			'simple-import-sort/imports': [
				'warn',
				{
					groups: [
						...
						['eslint'],
						[
							'^svelte\\u0000$', // import type {} from 'svelte'
...
const imports = combineConfig(importX, unusedImports, importSort)
